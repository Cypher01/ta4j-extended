...
public class HeikinAshiIndicator extends RecursiveCachedIndicator<Bar> {
  ...
  protected Bar calculate(int index) {
    ...
    Num haOpenPrice = prevHaBar.getOpenPrice().plus(prevHaBar.getClosePrice()).dividedBy(getBarSeries().numFactory().numOf(2));
    Num haClosePrice = openPrice.plus(highPrice).plus(lowPrice).plus(closePrice).dividedBy(getBarSeries().numFactory().numOf(4));
