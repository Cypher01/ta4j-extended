...
public class LogicIndicator extends CachedIndicator<Boolean> {
    ...
    public static LogicIndicator not(Indicator<Boolean> indicator) {
        return new LogicIndicator(Collections.singletonList(indicator),
                indicatorList -> indicatorList.stream().noneMatch(value -> value));
    }
    ...
    protected Boolean calculate(int index) {
        return transform.test(
                indicators.stream().map(indicator -> indicator.getValue(index)).collect(Collectors.toList()));
    }

    @Override
    public int getCountOfUnstableBars() {
        return indicators.stream().mapToInt(Indicator::getCountOfUnstableBars).max().orElse(0);
    }
}
