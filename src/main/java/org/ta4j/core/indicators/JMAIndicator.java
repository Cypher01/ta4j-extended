 ...
 * <a href="https://www.tradingview.com/script/nZuBWW9j-Jurik-Moving-Average/">TradingView</a>
 */
public class JMAIndicator extends CachedIndicator<Num> {
    ...
    private static class E0Indicator extends CachedIndicator<Num> {
        ...
        @Override protected Num calculate(int index) {
            ...
            return indicator.getValue(index).multipliedBy(getBarSeries().numFactory().numOf(1d - alpha)).plus(
                    getBarSeries().numFactory().numOf(alpha).multipliedBy(getValue(index - 1)));
        }
    ...
    private static class E1Indicator extends CachedIndicator<Num> {
        ...
        @Override protected Num calculate(int index) {
            if (index == 0) {
                return indicator.getValue(index).minus(e0Indicator.getValue(index)).multipliedBy(
                        getBarSeries().numFactory().numOf(1d - beta));
            }

            return indicator.getValue(index).minus(e0Indicator.getValue(index)).multipliedBy(
                    getBarSeries().numFactory().numOf(1d - beta)).plus(
                    getBarSeries().numFactory().numOf(beta).multipliedBy(getValue(index - 1)));
        }
    ...
    private static class E2Indicator extends CachedIndicator<Num> {
        ...
        public E2Indicator(E0Indicator e0Indicator, E1Indicator e1Indicator, JMAIndicator jmaIndicator,
                double phaseRatio, double alpha) {
        ...
        @Override protected Num calculate(int index) {
            if (index == 0) {
                return e0Indicator.getValue(index).plus(
                        e1Indicator.getValue(index).multipliedBy(getBarSeries().numFactory().numOf(phaseRatio))).minus(
                        nzPrevJmaIndicator.getValue(index)).multipliedBy(
                        getBarSeries().numFactory().numOf(1 - alpha).pow(2));
            }

            return e0Indicator.getValue(index).plus(
                    e1Indicator.getValue(index).multipliedBy(getBarSeries().numFactory().numOf(phaseRatio))).minus(
                    nzPrevJmaIndicator.getValue(index)).multipliedBy(
                    getBarSeries().numFactory().numOf(1 - alpha).pow(2)).plus(getBarSeries().numFactory()
                    .numOf(alpha)
                    .pow(2)
                    .multipliedBy(getValue(index - 1)));
        }
