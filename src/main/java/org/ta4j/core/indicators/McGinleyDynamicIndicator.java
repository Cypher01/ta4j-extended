...
public class McGinleyDynamicIndicator extends CachedIndicator<Num> {
    ...
    @Override protected Num calculate(int index) {
        ...
        return prevValue.plus(source.minus(prevValue)
                .dividedBy(getBarSeries().numFactory()
                        .numOf(barCount)
                        .multipliedBy(source.dividedBy(prevValue).pow(getBarSeries().numFactory().numOf(4)))));
    }
