 ...
 *
 * The implementation is simplified. Only the bull value is calculated. As the oscillator
 * is a percentage, the bear value can be calculated by 100 minus bull value.
 */
public class ASOIndicator extends AbstractIndicator<Num> {
	...
	public ASOIndicator(BarSeries series, int barCount) {
		...
		AverageIndicator intrabaravg = new AverageIndicator(CombineIndicator.minus(close, low), CombineIndicator.minus(high, open));
		CombineIndicator intrabarbulls = CombineIndicator.divide(TransformIndicator.multiply(intrabaravg, 100), k1);
		AverageIndicator groupavg = new AverageIndicator(CombineIndicator.minus(close, grouplow), CombineIndicator.minus(grouphigh, groupopen));
