...
public class SmoothedBarIndicator extends CachedIndicator<Bar> {
  ...
  public SmoothedBarIndicator(Indicator<Num> openPriceIndicator, Indicator<Num> highPriceIndicator, Indicator<Num> lowPriceIndicator, Indicator<Num> closePriceIndicator,
                              int barCount) {
  ...
  protected Bar calculate(int index) {
    ...
    return new BaseBarBuilder()
      ...
      .openPrice(smoothedOpen.getValue(index))
      .highPrice(smoothedHigh.getValue(index))
      .lowPrice(smoothedLow.getValue(index))
      .closePrice(smoothedClose.getValue(index))
