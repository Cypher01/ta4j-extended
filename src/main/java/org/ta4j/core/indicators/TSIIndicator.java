...
public class TSIIndicator extends AbstractIndicator<Num> {
	...
	public TSIIndicator(Indicator<Num> indicator, int longLength, int shortLength) {
		...
		EMAIndicator priceChangeDoubleSmoothed = new EMAIndicator(new EMAIndicator(priceChange, longLength), shortLength);;
		EMAIndicator priceChangeAbsDoubleSmoothed = new EMAIndicator(new EMAIndicator(TransformIndicator.abs(priceChange), longLength), shortLength);;

		this.tsiIndicator = TransformIndicator.multiply(CombineIndicator.divide(priceChangeDoubleSmoothed, priceChangeAbsDoubleSmoothed), 100);
