...
public class HEMAIndicator extends CachedIndicator<Num> {
	...
	protected Num calculate(int index) {
		...
		return one().minus(alpha).multipliedBy(getValue(index - 1).plus(bIndicator.getValue(index - 1))).plus(alpha.multipliedBy(indicator.getValue(index)));
	}
	...
	private static class BIndicator extends CachedIndicator<Num> {
		...
		protected Num calculate(int index) {
			...
			return one().minus(gamma).multipliedBy(getValue(index - 1)).plus(gamma.multipliedBy(hemaChangeIndicator.getValue(index)));
		}

		@Override
		public int getUnstableBars() {
			return hemaChangeIndicator.getUnstableBars();
		}
	}
}
