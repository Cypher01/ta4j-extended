...
public class HEMAIndicator extends CachedIndicator<Num> {
  ...
  protected Num calculate(int index) {
    ...
    return getBarSeries().numFactory().one()
                         .minus(alpha)
                         .multipliedBy(getValue(index - 1).plus(bIndicator.getValue(index - 1)))
                         .plus(alpha.multipliedBy(indicator.getValue(index)));
  }
  ...
  private static class BIndicator extends CachedIndicator<Num> {
    ...
    protected Num calculate(int index) {
      ...
      return getBarSeries().numFactory().one()
                           ...
                           .plus(gamma.multipliedBy(hemaChangeIndicator.getValue(index)));
    }
