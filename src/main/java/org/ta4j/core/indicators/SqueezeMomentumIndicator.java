 ...
 * The keltner channel indicators of ta4j-core are not used, because they use slightly different calculations (SMA vs.
 * EMA).
 */
public class SqueezeMomentumIndicator extends AbstractIndicator<Num> {
    ...
    public SqueezeMomentumIndicator(BarSeries series, int bbLength, double bbMultFactor, int kcLength,
            double kcMultFactor) {
        this(new ClosePriceIndicator(series), bbLength, bbMultFactor, kcLength, kcMultFactor);
    }

    public SqueezeMomentumIndicator(ClosePriceIndicator indicator, int bbLength, double bbMultFactor, int kcLength,
            double kcMultFactor) {
        super(indicator.getBarSeries());
        BarSeries series = indicator.getBarSeries();

        BollingerBandsMiddleIndicator middleBB = new BollingerBandsMiddleIndicator(
                new SMAIndicator(indicator, bbLength));
        StandardDeviationIndicator standardDev = new StandardDeviationIndicator(indicator, bbLength);
        this.upperBB = new BollingerBandsUpperIndicator(middleBB, standardDev,
                getBarSeries().numFactory().numOf(bbMultFactor));
        this.lowerBB = new BollingerBandsLowerIndicator(middleBB, standardDev,
                getBarSeries().numFactory().numOf(bbMultFactor));

        Indicator<Num> ma = new SMAIndicator(indicator, kcLength);
        Indicator<Num> range = TransformIndicator.multiply(new ATRIndicatorPlus(series, kcLength, Input.SMA),
                kcMultFactor);
        ...
        this.squeezeMomentumIndicator = new SimpleLinearRegressionIndicator(
                CombineIndicator.minus(indicator, averageIndicator), kcLength);
        this.unstableBars = Math.max(bbLength, kcLength);
    }

    public boolean squeezeOn(int index) {
        return (lowerBB.getValue(index).isGreaterThan(lowerKC.getValue(index))) && (upperBB.getValue(index).isLessThan(
                upperKC.getValue(index)));
    }

    public boolean squeezeOff(int index) {
        return (lowerBB.getValue(index).isLessThan(lowerKC.getValue(index))) && (upperBB.getValue(index).isGreaterThan(
                upperKC.getValue(index)));
    }
