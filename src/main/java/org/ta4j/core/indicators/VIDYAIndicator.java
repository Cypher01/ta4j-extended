...
public class VIDYAIndicator extends CachedIndicator<Num> {
    ...
    @Override protected Num calculate(int index) {
        ...
        return indicator.getValue(index).multipliedBy(alpha).multipliedBy(cmoAbsNormValue).plus(
                prevValue.multipliedBy(getBarSeries().numFactory().one().minus(alpha.multipliedBy(cmoAbsNormValue))));
    }
